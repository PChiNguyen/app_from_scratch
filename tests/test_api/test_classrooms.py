...
def test_create_classroom_fail(client: TestClient):
    ...
    classroom_data = ClassroomCreate(name=Type.number1_, teacher_id=uuid.uuid4())
...
def test_create_classroom_fail_wrong_name(client: TestClient, mock_user: User):
    # 1. Prepare request payload
    classroom_data = ClassroomCreate(name='six seven', teacher_id=mock_user.id)   
